...
fn render_events(events: &[ReplEvent]) -> String {
    events
        ...
        .filter_map(|event| match event {
            ReplEvent::Output(text) => Some(text.as_str()),
            ReplEvent::Exit => None,
        })
        .collect::<Vec<_>>()
        .join("\n")
...
fn consolidation_keeps_durable_drops_ephemeral() {
    ...
    // Durable convention kept.
    assert_round_trips(config_dir.path(), project_dir.path(), "pnpm");
    // One-off task details dropped — these must NOT have been persisted.
    assert!(
        !store.contains("line 42"),
        "ephemeral 'line 42' task detail was persisted as durable memory:\n{store}"
    );
    assert!(
        !store.contains("staging"),
        "ephemeral 'restart staging' task detail was persisted as durable memory:\n{store}"
    );
}
